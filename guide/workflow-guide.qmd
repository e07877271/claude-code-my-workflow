...
| Bash safety | `PreToolUse[Bash]` | Blocks `rm -rf`, force push, `sudo`, curl pipe to shell |
| Output scanner | `PostToolUse` | Warns if tool output contains leaked secrets |
| Audit log | `PostToolUse` | Append-only JSONL audit trail of all tool invocations |
| Pipeline isolation | `PreToolUse[Edit\|Write]` | Reviewer agents can only write to `quality_reports/` |
| Foreground agents | `PreToolUse[Agent]` | Blocks background agents that can't prompt for permissions |
| Plan persistence | `Stop` | Blocks if plan discussed but not saved to disk |
...
The template includes a defense-in-depth security layer:

- **Static deny list** in `settings.json` blocks 17 dangerous patterns at the permission level
- **`bash-safety.sh`** provides dynamic pattern matching for destructive commands (`rm -rf`, force push, `sudo`, curl pipe to shell)
- **`output-scanner.sh`** detects leaked secrets (AWS keys, API tokens, private keys) in tool output
- **`audit-log.sh`** maintains an append-only JSONL trail of all tool invocations

Safety hooks fail CLOSED (block on error) unlike monitoring hooks which fail OPEN (allow on error). This means if a safety hook crashes, the operation is blocked --- the safe default. Monitoring hooks (like the session log reminder) allow the operation to proceed even if the hook itself fails.
...
### Pattern 16: Second-Opinion with Oracle {#pattern-16-oracle}

For complex problems that benefit from independent verification --- structural estimation plans, theorem proofs, identification arguments --- use the Oracle skill to get a second opinion from ChatGPT Pro:

```
/oracle Is this identification argument for the LATE valid under heterogeneous treatment effects?
```

Oracle bundles your project context + prompt into a single rich query. Browser mode takes 10--60 minutes but gives access to extended thinking. Use `--render --copy` as a fallback to manually paste into ChatGPT.

**When to use:** Plan reviews for complex empirical projects, proving theorems, extending models from papers.

### Pattern 17: PDF Preprocessing Pipeline {#pattern-17-pdf-preprocessing}

Before feeding papers to `/lit-review`, `/review-paper`, or `/oracle`, preprocess them with LiteParse:

```
/parse-paper master_supporting_docs/angrist_pischke_2009.pdf
```

Use text extraction for prose and references. Use screenshots for tables and equations (text extraction mangles math). Batch-process your entire reading list for a literature review.

### Pattern 18: Autonomous Research Loops (Autoresearch) {#pattern-18-autoresearch}

For Monte Carlo simulations and parameter exploration, use the Karpathy autoresearch pattern with `/simulation-study`:

1. Create a `program.md` file (use `templates/research-program.md`) defining:
   - What files the agent CAN modify (estimator implementations, parameters)
   - What is FROZEN (data generation pipeline, evaluation metrics)
   - A single scalar success metric (RMSE, coverage, etc.)
2. Run `/simulation-study` to scaffold the harness
3. Let the agent iterate: modify → run → measure → keep or revert → repeat

The key insight: reduce success to a number. If you can't define "better" numerically, the autonomous loop can't help.

### Pattern 19: Pipeline Isolation {#pattern-19-pipeline-isolation}

Reviewer agents are read-only by design (enforced by the `enforce-isolation.sh` hook). This prevents the common failure mode where a reviewer who also fixes finds only easily fixable issues.

The workflow: reviewer identifies issues → documents in report → orchestrator dispatches fixer → fixer implements fixes → reviewer re-verifies → checks for false convergence.

This is the StatsClaw principle: "Critics never create. Creators never self-score."
...
| Cross-model verification | Oracle CLI | Get a second opinion from ChatGPT Pro for proofs, identification, and strategy ([Pattern 16](#pattern-16-oracle)) |
...
| `/oracle` | `.claude/skills/oracle/` | Cross-validate with ChatGPT Pro via Oracle CLI |
| `/parse-paper` | `.claude/skills/parse-paper/` | Extract structured content from academic PDFs |
| `/mailbox` | `.claude/skills/mailbox/` | Structured inter-agent communication |
| `/progress` | `.claude/skills/progress/` | Visual progress tracking |
| `/ship` | `.claude/skills/ship/` | One-command commit-push-PR-merge |
| `/simulation-study` | `.claude/skills/simulation-study/` | Monte Carlo simulation scaffold |
...
| Bash safety | PreToolUse[Bash] (command) | `.claude/hooks/bash-safety.sh` |
| Output scanner | PostToolUse (command) | `.claude/hooks/output-scanner.sh` |
| Audit log | PostToolUse (command) | `.claude/hooks/audit-log.sh` |
| Pipeline isolation | PreToolUse[Edit\|Write] (command) | `.claude/hooks/enforce-isolation.sh` |
| Foreground agents | PreToolUse[Agent] (command) | `.claude/hooks/foreground-agents.sh` |
| Plan persistence | Stop (command) | `.claude/hooks/plan-persistence.sh` |
